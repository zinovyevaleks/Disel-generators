...
var MESSAGES = {
  ...
  "photoviewer.toolbar.first":       "Начало",
  "photoviewer.toolbar.prev":        "Предыдущая",
  "photoviewer.toolbar.slideShow":   "Старт/Стор Слайд шоу (Пробел)",
  "photoviewer.toolbar.next":        "Следуюшая",
  "photoviewer.toolbar.last":        "Конец",
  "photoviewer.toolbar.email":       "Отправить по почте",
  "photoviewer.toolbar.permalink":   "Ссылка",
  "photoviewer.toolbar.close":       "Закрыть (Esc)",
  "photoviewer.email.subject.photo": "Фотография",
...
function checkDomain(){
	try{
		...
		if(protocol.toLowerCase().indexOf("file")===0){return true;}
		...
		pos=domain.indexOf("://");
		if(pos!=-1){domain=domain.substring(pos+3);}
		pos=domain.indexOf("/");
		if(pos!=-1){domain=domain.substring(0,pos);}
		if(domain.indexOf("www.")===0){domain=domain.substring(4);}
		if(host==domain){return true;}
		else if(new RegExp(domain).test(host)){return true;}
		return false;
	}catch(e){return true;}
}
...
function viewerCloseCallback(photoIndex){
	...
	for(i=0;i<gallery.sets.length;i++){
		for(j=0;j<gallery.sets[i].photos.length;j++){
			...
			if(p==undefined||p.orig==undefined||p.orig.src==undefined)continue;
			if(n==photoIndex){
				gallery.setIndex=i;
...
var OFFSET_TOP=0;
var REST_URL="/rest/";
var P_IMG_ROOT="/design/images/tt";
...
function addPhoto(photo,title,time,byline,link,alt){var type=typeof photo;if(typeof photo=="string"){photo=new PhotoImg(undefined,photo,undefined,undefined,time,title,byline,link,alt);}
...
function showPhoto(index,cropWidth,opacity){
	...
	if(this.index<0||this.index>=this.photos.length){log.error("Invalid photo index");return true;}
	var doc=this.win.document;
	var firstShow=false;
	if(!this.shown){firstShow=true;doc.viewer=this;
	try{this.hideOverlappingElements();}catch(e){}}
	var zIndex=16384;
	...
	if(isNaN(photo.w)||isNaN(photo.h)){
		if(photo.preloadImage!=undefined){
			...
			this.index--;this.next();return false;
		}
	}
	if(isNaN(photo.w)||isNaN(photo.h)){
		this.index--;this.next();return false;
	...
	}else{pw=parseInt(photo.w);ph=parseInt(photo.h);}
	if(pw<=0||ph<=0){
	...
	if(this.photos.length>1){
		...
		//photoElt.onclick=function(){getViewer().next()};
	}
	...
	$(photoElt).attr('alt',photo.title);
	$(photoElt).attr('title',photo.title);
	...
	if(viewer.photoDiv!=undefined){
		try{doc.body.removeChild(viewer.photoDiv);}catch(e){}
	}
...
function closePhoto(){
	...
	if(this.contextMenu!=undefined) this.contextMenu.hide();
	//$('body').css('overflow','scroll');	//modified by HAWK
	//$('body').css('overflow','auto').unbind('mousewheel');
...
function fadePhoto(fadeIn,inProgress,callback){
	...
	if(!fadeIn){
		...
		if(this.fadeDegree>0){
			...
			if(slideFadingTimeout)window.clearTimeout(slideFadingTimeout);
			slideFadingTimeout=window.setTimeout(function(){viewer.fadePhoto(false,true,callback);},50);return;
		}else{setOpacity(fadeElt,0);}
		if(callback) callback();*/
		$(fadeElt).animate({opacity:0},200,'swing',function(){
			if(callback) callback();
			viewer.fadePhoto(true,false,callback);
		});
	}else{
		...
		if(this.fadeDegree<1.0){
			setOpacity(fadeElt,this.fadeDegree);
			if(slideFadingTimeout)window.clearTimeout(slideFadingTimeout);
			slideFadingTimeout=window.setTimeout(function(){viewer.fadePhoto(true,true);},50);return;
		}else{setOpacity(fadeElt,1.0);}
		this.viewerFading=false;
		if(this.showCallback!=undefined) this.showCallback(this.index);*/
		$(fadeElt).css('opacity',0).animate({opacity:1},200,'swing',function(){
			if(this.showCallback!=undefined) this.showCallback(this.index);
		});
	}
}
...
function addBackShade(zIndex){
	var doc=this.win.document;
	if(doc.getElementById(VIEWER_ID_BACK)!=undefined){return;}
	...
	//$('body').css('overflow','hidden');	//modified by HAWK
	//$('body,html').mousewheel(function(){return false;});
	//$('body').mousewheel(function(){return false;});
	//$(photoBack).css({'top':$(window).scrollTop(),'left':$(window).scrollLeft(),'height':$(window).height(),'width':$(window).width()});
	$(photoBack).css({'top':0,'left':0,'height':'100%','width':'100%'});
...
function addToolbar(availW,zIndex){
	...
	if(!this.overrideToolbarStyles){
		...
		var top=getWindowSize(this.win).h+$(window).scrollTop();
		...
		photoToolbar.style.lineHeight=LINE_HEIGHT;photoToolbar.style.border="none";
	}
	...
	if(imgMap===undefined){
		...
		if(this.enableEmailLink)areas.push(["getViewer().email()","300",getMessage("photoviewer.toolbar.email")]);
		if(this.isEnablePhotoLink)areas.push(["getViewer().permalink()","350",getMessage("photoviewer.toolbar.permalink")]);
		areas.push(["getViewer().close()","402",getMessage("photoviewer.toolbar.close")]);
		for(i=0;i<areas.length;i++){
			var area=doc.createElement("area");
			if(!ssl)area.href="javascript:void(0)";
			area.alt=areas[i][2];
			area.title=area.alt;
			area.shape="circle";
	...
	$(window).resize(function(){
		//if(IE)	$('#'+VIEWER_ID_TOOLBAR).css({'left':($(window).scrollLeft()+$(window).width()-$('#'+VIEWER_ID_TOOLBAR).width()+10)/2,'top':($(window).scrollTop()+$(window).height()-$('#'+VIEWER_ID_TOOLBAR).height()-10)+"px"});
		//else $('#'+VIEWER_ID_TOOLBAR).css({'left':($(window).width()-$('#'+VIEWER_ID_TOOLBAR).width()+10)/2,'top':($(window).height()-$('#'+VIEWER_ID_TOOLBAR).height()-10)+"px"});
		//$('#'+VIEWER_ID_BACK).css({'top':$(window).scrollTop(),'left':$(window).scrollLeft(),'height':$(window).height(),'width':$(window).width()});
		var top=getWindowSize(this).h+$(window).scrollTop();
		var left=($(window).scrollLeft()+getWindowSize(this).w-$('#'+VIEWER_ID_TOOLBAR).width()+10)/2;
		$('#'+VIEWER_ID_TOOLBAR).css({'top':(top-TOOLBAR_H-10)+"px",'left':left+"px"});
		var left=parseInt(($(window).width()-$('#'+VIEWER_ID_PHOTO).width())/2)-10;
		var top=parseInt(($(window).height()-$('#'+VIEWER_ID_PHOTO).height())/2)+$(window).scrollTop()-10;
		$('#'+VIEWER_ID_PHOTO).css({'left':left,'top':top});
	});
}
...
function onclickEvent(){
	var v=getViewer();
	if(v.contextMenu!=undefined&&v.contextMenu.visible){v.contextMenu.hide();return;}
	if(v.toolbarAnimator!=undefined) v.toolbarAnimator.reset();
	if(v.customOnclickEvent!=undefined) v.customOnclickEvent();else
	closeViewer();
}
...
$(function(){
	...
	if($('div.ShowPhoto, span.ShowPhoto, ul.ShowPhoto').length){
		$('div.ShowPhoto, span.ShowPhoto, ul.ShowPhoto').each(function(index){
			viewers[index]=new PhotoViewer();
			viewers[index].enableFading();
			viewers[index].setBorderWidth(6);
			viewers[index].disableEmailLink();
			viewers[index].disablePhotoLink();
			viewers[index].enableLoop();
			viewers[index].setFontSize(14);
			viewers[index].enableToolbarAnimator();
			viewers[index].disablePanning();
			viewers[index].enablePhotoFading();
			viewers[index].setShadeOpacity(0.95);
			$('a.ShowPhoto',this).each(function(index2){
				$(this).attr('viewer',index);
				$(this).attr('num',index2);
				viewers[index].add($('img',this).attr('fullsrc'),$('img',this).attr('title'));
				$(this).unbind('click').click(function(){
					viewers[$(this).attr('viewer')].show(Number($(this).attr('num')));
					return false;
				});
			});
		});
...
// Скрипт масштабирования и таскания картинки - Добавил Березин А.С. - start
$("#PhotoViewerBack").on("wheel", function() {
	return false;
});

$("#PhotoViewerToolbar").on("wheel", function() {
	return false;
});

$("#PhotoViewerPhoto").on("wheel", function() {
	return false;
});

$("img","#PhotoViewerPhoto").on("wheel", function(e) {
	$("#PhotoViewerPhoto").on("mouseout", kyoslider_fix);
	$("#PhotoViewerPhoto").on("mouseover", kyoslider_fix);
	$("#PhotoViewerPhoto").click("mouseover", kyoslider_fix);

	$(this).draggable();
	
	kyoscale_width = $(this).width();
	kyoscale_height = $(this).height();
	
	kyoscale_container_width = $(this).parent().width();
	kyoscale_container_height = $(this).parent().height();
	
	if (e.originalEvent.wheelDelta /120 > 0) {
		kyoscale_width = kyoscale_width / 100;
		kyoscale_width = kyoscale_width * 120;
		kyoscale_height = kyoscale_height / 100;
		kyoscale_height = kyoscale_height * 120;
	}	else {
		kyoscale_width = kyoscale_width / 100;
		kyoscale_width = kyoscale_width * 80;
		kyoscale_height = kyoscale_height / 100;
		kyoscale_height = kyoscale_height * 80;
		if (kyoscale_width < kyoscale_container_width) {
			kyoscale_width = kyoscale_container_width;
			kyoscale_height = kyoscale_container_height;
		};
	};
	
	kyoscale_width = Math.floor(kyoscale_width);
	kyoscale_width = Math.ceil(kyoscale_width); 
	kyoscale_width = Math.round(kyoscale_width);
	
	kyoscale_height = Math.floor(kyoscale_height);
	kyoscale_height = Math.ceil(kyoscale_height); 
	kyoscale_height = Math.round(kyoscale_height);
	
	kyoscale_width = kyoscale_width + "px";
	kyoscale_height = kyoscale_height + "px";
	
	$(this).css("width",kyoscale_width);
	$(this).css("height",kyoscale_height);
	
	return false;
});

function kyoslider_fix () {
	kyoscale_container_width = $("#PhotoViewerPhoto").width();
	kyoscale_container_height = $("#PhotoViewerPhoto").height();
	kyoscale_image_width = $("img","#PhotoViewerPhoto").width();
	kyoscale_image_height = $("img","#PhotoViewerPhoto").height();

	kyoscale_left = $("img","#PhotoViewerPhoto").css("left");
	kyoscale_top = $("img","#PhotoViewerPhoto").css("top");
	
	kyoscale_container_width = kyoscale_container_width - 10;
	kyoscale_container_height = kyoscale_container_height - 10;
	kyoscale_image_width = kyoscale_image_width - 10;
	kyoscale_image_height = kyoscale_image_height - 10;
	
	kyoscale_left = parseInt(kyoscale_left,10);
	kyoscale_top = parseInt(kyoscale_top,10);
	
	if (kyoscale_left > kyoscale_image_width || kyoscale_top > kyoscale_image_height) {
		$("img","#PhotoViewerPhoto").css("left","auto");
		$("img","#PhotoViewerPhoto").css("top","auto");
	};
	
	kyoscale_left = kyoscale_left * -1;
	kyoscale_top = kyoscale_top * -1;
	
	if (kyoscale_left > kyoscale_image_width || kyoscale_top > kyoscale_image_height) {
		$("img","#PhotoViewerPhoto").css("left","auto");
		$("img","#PhotoViewerPhoto").css("top","auto");
		$("img","#PhotoViewerPhoto").css("width",kyoscale_container_width);
		$("img","#PhotoViewerPhoto").css("height",kyoscale_container_height);
	};
};
// Скрипт масштабирования и таскания картинки - Добавил Березин А.С. - end